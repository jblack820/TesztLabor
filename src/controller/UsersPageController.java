...
//</editor-fold>    
    ...
    static void handleEditRequest(String userKey, TableCell tc) {
        ...
        if (userKey.equalsIgnoreCase(MainController.getUserLoggedIn().getUserKey())
                && MainController.getUserLoggedIn().hasRole(UserRole.ADMIN)) {
        ...
        if (!MainController.userLoggedIn.hasRole(UserRole.ADMIN)) {
            editUserKey.setDisable(true);
    ...
    private void handleModifyButtonClicked(ActionEvent event) {
        ...
        Main.controller.reScanUsersJSON();
        MainController.setUserLoggedIn(Main.controller.getUsers().getUserByUserKey(System.getProperty("user.home")));
        try {
            goToUsersPage(event);
...
//</editor-fold>
    ...
    private boolean isUserAdmin() {
        return MainController.userLoggedIn.getRoleList().contains(UserRole.ADMIN);
    }
//</editor-fold>
}
